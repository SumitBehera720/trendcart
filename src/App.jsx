...
export default function App() {
  ...
  const [pastOrders, setPastOrders] = useState(() => JSON.parse(localStorage.getItem('trendcart_orders') || '[]'));
      ...
      if (token) {
        try {
          ...
          if (user && Object.keys(user).length > 0) {
            ...
            setCurrentUser(null);
            localStorage.removeItem('trendcart_auth_token');
  ...
  const handlePaymentSuccess = async (paymentDetails) => {
    ...
    try {
      ...
      if (currentUser) {
        ...
        localStorage.setItem('trendcart_orders', JSON.stringify([newOrder, ...pastOrders]));
      }
  ...
  const handleLogin = async (user, token) => {
    if (token) localStorage.setItem('trendcart_auth_token', token);
    setCurrentUser(user);
  ...
  const handleSeedMockOrders = (itemCount) => {
    ...
    const orderId1 = `TC-${Math.floor(100000 + Math.random() * 900000)}`;
    const orderId2 = `TC-${Math.floor(100000 + Math.random() * 900000)}`;
    ...
    const order1 = {
      ...
      subtotal: 499 * Math.ceil(itemCount / 2),
      shipping: 0,
    ...
    const order2 = {
      ...
      subtotal: 899 * Math.floor(itemCount / 2),
      shipping: 0,
      stepVal: 5 // Delivered
    };
    ...
    localStorage.setItem('trendcart_orders', JSON.stringify(newOrders));
  };
  ...
  return (
    <div className="app-container">
      ...
      {/* Header */}
          <header className={`site-header ${isHeaderScrolled ? 'scrolled' : ''}`}>
            <div className="container header-container">
              ...
              <a href="#" className="logo-link" onClick={(e) => { e.preventDefault(); scrollToSection('hero'); }}>
                <span className="logo-text" style={{ background: 'var(--accent-gradient)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', display: 'inline-block' }}>🛍️ TrendCart</span>
                <span className="logo-sub">SHOP SMART, LIVE BETTER</span>
              </a>
          ...
          <a 
            href="https://wa.me/919876543210?text=Hi%20TrendCart%20Support,%20I%20have%20an%20inquiry%20regarding%20my%20recent%20order." 
            target="_blank" 
